...
/****************************机器人交互数据****************************/
/* 发送的内容数据段最大为 113
 * 检测是否超出大小限制?实际上图形段不会超，数据段最多30个，也不会超*/
/* 交互数据头结构 */
typedef struct {
...
typedef struct {
  uint16_t data_cmd_id; // 由于存在多个内容 ID，但整个cmd_id 上行频率最大为
                        // 10Hz，请合理安排带宽。注意交互部分的上行频率
  uint16_t sender_ID;
...
typedef struct {
  ...
  uint8_t joint6_sign;     /* payload byte 27: 0 = non-negative, 1 = negative */
  uint8_t reserved[5];     /* payload bytes 28..30: padding (0) */
